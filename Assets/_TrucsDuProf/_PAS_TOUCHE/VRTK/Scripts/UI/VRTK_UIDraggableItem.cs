...
namespace VRTK
{
    ...
    public class VRTK_UIDraggableItem : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
    {
        ...
        {
//          ****Custom****
//          Delete object if dropped too far from drop zone
            if (GetComponent<ItemDragHandler>())
            {
                ItemDragHandler drag = GetComponent<ItemDragHandler>();

                if (Vector2.Distance(startPosition, transform.position) >= drag.dragDistanceDestroy)
                {
                    if (drag.sloted)
                    {
                        drag.ResetDestroy();
